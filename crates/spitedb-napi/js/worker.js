...
async function main() {
    ...
    if (typeof projection.getTenantId !== 'function') {
        throw new Error(`Projection "${projection.name}" missing getTenantId function.\n` +
    ...
    while (true) {
        try {
            ...
            const tenantProxies = new Map();
            // Get or create proxy for a tenant
            const projName = projection.name; // Capture for closure
            function getProxyForTenant(tenantId) {
                let proxyData = tenantProxies.get(tenantId);
                if (!proxyData) {
                    const { proxy, flush } = createProjectionProxy(projName, tenantId, primaryKey, db);
