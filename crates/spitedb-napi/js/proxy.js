 ...
 */
export function createProjectionProxy(projectionName, tenantId, primaryKeyColumn, native) {
     ...
     * Tenant isolation is enforced - only this tenant's data is accessible.
     */
    function readRow(key) {
        ...
        // Read from database with tenant scoping
        const json = native.readProjectionRow(projectionName, tenantId, key);
        if (json === null) {
            readCache.set(key, null);
            return null;
        }
