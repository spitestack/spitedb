 ...
 *       const row = table[userId] ?? { login_count: 0 };
 *       row.login_count++;
 *       table[userId] = row;
 *     }
 *   },
...
export interface ProjectionDefinition<TSchema extends SchemaDefinition = SchemaDefinition> {
   ...
   *
   * Note: A `tenant_id` column is automatically added as part of the
   * composite primary key for tenant isolation.
   */
  schema: TSchema;

  /**
   * Extracts the tenant ID from an event.
   *
   * This is REQUIRED for tenant isolation. The framework uses this to ensure
   * complete data isolation between tenants. It's impossible to accidentally
   * read or write another tenant's data.
   *
   * Common patterns:
   * - Extract from stream_id: `event.streamId.split('-')[1]`
   * - Parse from event data: `JSON.parse(event.data).tenantId`
   *
   * @example
   * ```typescript
   * getTenantId: (event) => event.streamId.split('-')[1] // For "Order-tenant123-order456"
   * ```
   */
  getTenantId: (event: ProjectionEvent) => string;
   ...
   *
   * The table is automatically scoped to the tenant extracted by getTenantId().
