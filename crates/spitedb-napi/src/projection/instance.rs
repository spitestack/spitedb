...
use super::schema::{json_to_sql_literal, json_value_to_sql, row_value_to_json, ProjectionSchema, TENANT_ID_COLUMN};
...
impl ProjectionInstance {
    ...
    /// Reads a row by tenant_id and primary key.
    ///
    /// Returns the row as a JSON object, or None if not found.
    /// The tenant_id is required to ensure tenant isolation.
    pub fn read_row(&self, tenant_id: &str, key: &str) -> Result<Option<JsonValue>, ProjectionError> {
        let user_pk_name = self
            .schema
        ...
        let sql = format!(
            ...
            self.schema.table_name, TENANT_ID_COLUMN, user_pk_name
        );

        let mut stmt = self.conn.prepare(&sql)?;

        let result = stmt
            .query_row([tenant_id, key], |row| {
    ...
    fn apply_batch_inner(
        ...
        for op in operations {
            match op.op_type {
                OpType::Upsert => {
                    self.execute_upsert(user_pk_name, &op.tenant_id, &op.key, op.value.as_ref())?;
                }
                OpType::Delete => {
                    self.execute_delete(user_pk_name, &op.tenant_id, &op.key)?;
                }
            }
        }
    ...
    fn execute_upsert(
        ...
        // Build column list and values - tenant_id first, then user pk
        let mut columns = vec![TENANT_ID_COLUMN.to_string(), user_pk_name.to_string()];
        ...
        if let JsonValue::Object(obj) = value {
            for col in &self.schema.columns {
                // Skip tenant_id and user pk - already handled
                if col.name == TENANT_ID_COLUMN || col.name == user_pk_name {
                    continue;
                }
    ...
    fn execute_delete(&self, user_pk_name: &str, tenant_id: &str, key: &str) -> Result<(), ProjectionError> {
        let sql = format!(
            "DELETE FROM {} WHERE {} = ? AND {} = ?",
            self.schema.table_name, TENANT_ID_COLUMN, user_pk_name
        );
        self.conn.execute(&sql, [tenant_id, key])?;
        Ok(())
    }

    /// Deletes all rows for a tenant (for GDPR compliance).
    ///
    /// This is called when a tenant is deleted to cascade the deletion
    /// to all projection data.
    pub fn delete_tenant(&mut self, tenant_id: &str) -> Result<u64, ProjectionError> {
        let sql = format!(
            "DELETE FROM {} WHERE {} = ?",
            self.schema.table_name, TENANT_ID_COLUMN
        );
        let rows_deleted = self.conn.execute(&sql, [tenant_id])? as u64;
        Ok(rows_deleted)
    }
}
...
mod tests {
    ...
    #[test]
    fn test_upsert_and_read_with_tenant() {
        let temp_dir = TempDir::new().unwrap();
        let schema = create_test_schema();
        ...
        // Read with correct tenant - should find it
        let row = instance.read_row(tenant_id, "key1").unwrap();
        assert!(row.is_some());

        let obj = row.unwrap();
        assert_eq!(obj["tenant_id"], tenant_id);
        assert_eq!(obj["id"], "key1");
        assert_eq!(obj["count"], 42);

        // Read with wrong tenant - should NOT find it (tenant isolation)
        let wrong_tenant_row = instance.read_row("other-tenant", "key1").unwrap();
        assert!(wrong_tenant_row.is_none());
    }
    ...
    fn test_tenant_isolation() {
        ...
        // Insert same key for two different tenants
        let ops = vec![
            ProjectionOp {
                op_type: OpType::Upsert,
                tenant_id: "tenant-a".to_string(),
                key: "user-1".to_string(),
                value: Some(serde_json::json!({"count": 100})),
            },
            ProjectionOp {
                op_type: OpType::Upsert,
                tenant_id: "tenant-b".to_string(),
                key: "user-1".to_string(),
                value: Some(serde_json::json!({"count": 200})),
            },
        ];
        instance.apply_batch(ops, 1).unwrap();

        // Each tenant sees their own data
        let row_a = instance.read_row("tenant-a", "user-1").unwrap().unwrap();
        assert_eq!(row_a["count"], 100);

        let row_b = instance.read_row("tenant-b", "user-1").unwrap().unwrap();
        assert_eq!(row_b["count"], 200);
    }

    #[test]
    fn test_delete_with_tenant() {
        let temp_dir = TempDir::new().unwrap();
        let schema = create_test_schema();
        ...
        // Verify it exists
        assert!(instance.read_row(tenant_id, "key1").unwrap().is_some());
        ...
        // Verify it's gone
        assert!(instance.read_row(tenant_id, "key1").unwrap().is_none());
    }

    #[test]
    fn test_delete_tenant_gdpr() {
        let temp_dir = TempDir::new().unwrap();
        let schema = create_test_schema();
        ...
        let ops = vec![
            ProjectionOp {
                op_type: OpType::Upsert,
                tenant_id: "tenant-to-delete".to_string(),
                key: "key1".to_string(),
                value: Some(serde_json::json!({"count": 1})),
            },
            ProjectionOp {
                op_type: OpType::Upsert,
                tenant_id: "tenant-to-delete".to_string(),
                key: "key2".to_string(),
                value: Some(serde_json::json!({"count": 2})),
            },
            ProjectionOp {
                op_type: OpType::Upsert,
                tenant_id: "tenant-to-keep".to_string(),
                key: "key1".to_string(),
                value: Some(serde_json::json!({"count": 100})),
            },
        ];
        ...
        // Delete tenant
        let deleted = instance.delete_tenant("tenant-to-delete").unwrap();
        assert_eq!(deleted, 2);

        // Verify tenant data is gone
        assert!(instance.read_row("tenant-to-delete", "key1").unwrap().is_none());
        assert!(instance.read_row("tenant-to-delete", "key2").unwrap().is_none());

        // Verify other tenant's data is preserved
        assert!(instance.read_row("tenant-to-keep", "key1").unwrap().is_some());
    }
}
