...
//! # Tenant Scoping
//!
//! All projections automatically include a `tenant_id` column as the first
//! column. This column is part of the composite primary key, ensuring:
//! - Complete tenant isolation (impossible to read across tenants)
//! - No developer error (framework enforces scoping)
//! - Simple GDPR deletion (delete all rows for a tenant)

use serde_json::Value as JsonValue;

use crate::ColumnDefNapi;

/// Column name for tenant isolation - framework-enforced.
pub const TENANT_ID_COLUMN: &str = "tenant_id";
...
impl ProjectionSchema {
    /// Creates a new projection schema with tenant_id automatically prepended.
    ///
    /// This ensures all projections are tenant-scoped by default.
    /// The tenant_id becomes part of the composite primary key.
    pub fn new_with_tenant(table_name: String, mut columns: Vec<ColumnDef>) -> Self {
        // Prepend tenant_id column as primary key
        let tenant_col = ColumnDef {
            name: TENANT_ID_COLUMN.to_string(),
    ...
    /// Gets the user-defined primary key column name (excludes tenant_id).
    pub fn user_primary_key_name(&self) -> Option<&str> {
        self.columns
            .iter()
            .find(|c| c.primary_key && c.name != TENANT_ID_COLUMN)
            .map(|c| c.name.as_str())
    }

    /// Gets the primary key column name (assumes single PK).
    /// Deprecated: Use primary_key_names() for composite keys.
