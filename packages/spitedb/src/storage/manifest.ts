 ...
 *
 * // Add a segment
 * manifest.addSegment({ id: 1n, path: 'segment-00000001.log', basePosition: 0n });
 *
 * // Save atomically
 * await manifest.save();
 * ```
...
interface ManifestData {
  ...
  segments: Array<{
    ...
    path: string;
    basePosition: string; // bigint as string
  }>;
}
...
export class Manifest {
  ...
  async load(dataDir: string): Promise<boolean> {
    ...
    try {
      ...
      // Parse manifest
      this.globalPosition = BigInt(data.globalPosition);
      ...
      for (const seg of data.segments) {
        const segment: ManifestSegment = {
          id: BigInt(seg.id),
          path: seg.path,
          basePosition: BigInt(seg.basePosition),
        };
  ...
  async save(): Promise<void> {
    ...
    const tempPath = `${this.dataDir}/${MANIFEST_TEMP_FILENAME}`;
   ...
   * Set global position.
   */
  setGlobalPosition(position: bigint): void {
