...
import { Manifest } from '../../../src/storage/manifest';
...
describe('Manifest', () => {
  ...
  describe('load', () => {
    ...
    it('should load existing manifest', async () => {
      ...
      expect(manifest.getGlobalPosition()).toBe(100n);
  ...
  describe('save', () => {
    it('should save manifest atomically', async () => {
      ...
      manifest.addSegment({ id: 0n, path: 'segment-00000000.log', basePosition: 0n });
      manifest.setGlobalPosition(50n);
      ...
      expect(loaded).toBe(true);
      expect(manifest2.getGlobalPosition()).toBe(50n);
    ...
    it('should preserve segment data through save/load cycle', async () => {
      ...
      manifest.addSegment({ id: 0n, path: 'segment-00000000.log', basePosition: 0n });
      manifest.addSegment({ id: 1n, path: 'segment-00000001.log', basePosition: 100n });
      manifest.addSegment({ id: 2n, path: 'segment-00000002.log', basePosition: 200n });
      manifest.setGlobalPosition(300n);
      ...
      expect(segments[0]!.basePosition).toBe(0n);
      expect(segments[1]!.id).toBe(1n);
      expect(segments[1]!.basePosition).toBe(100n);
      expect(segments[2]!.id).toBe(2n);
      expect(segments[2]!.basePosition).toBe(200n);
    });
  });
  ...
  describe('segment management', () => {
    ...
    it('should auto-update nextSegmentId when adding segment', () => {
      ...
      manifest.addSegment({ id: 5n, path: 'segment-00000005.log', basePosition: 0n });
  ...
  describe('state management', () => {
    ...
    it('should track global position', () => {
      expect(manifest.getGlobalPosition()).toBe(0n);

      manifest.setGlobalPosition(100n);
      expect(manifest.getGlobalPosition()).toBe(100n);
    });
    ...
    it('should track dirty state', () => {
      ...
      manifest.addSegment({ id: 0n, path: 'test.log', basePosition: 0n });
  ...
  describe('initializeFromScan', () => {
    ...
    it('should initialize from scan results', () => {
      manifest.initializeFromScan(
        [
          { id: 0n, path: 'segment-00000000.log', basePosition: 0n },
          { id: 1n, path: 'segment-00000001.log', basePosition: 100n },
        ],
        200n,
        2n
      );
      ...
      expect(manifest.getSegmentCount()).toBe(2);
      expect(manifest.getGlobalPosition()).toBe(200n);
