...
describe('Flush Failure Idempotency', () => {
  ...
  describe('position tracking', () => {
    test('durable position should track successful flushes only', async () => {
      ...
      // Durable position should not advance on failed flush
      // (This depends on implementation - currently it might advance)
      // The important thing is events are not duplicated

      // Fix faults and retry
      env.fs.clearFaults();
      await eventStore.flush();
