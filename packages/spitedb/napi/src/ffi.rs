...
//!
//! This module provides a thin C ABI layer for use with Bun's bun:ffi module.
...
use spitedb::{AppendCommand, CommandId, EventData, SpiteDB, StreamId, StreamRev, Tenant};

/// Opaque handle to a SpiteDB instance
...
pub struct FfiAppendResult {
    ...
    pub first_rev: u64,
    pub last_rev: u64,
    pub error: *mut c_char, // null if success, otherwise error message (caller must free)
}
...
/// Returns null on error (check last error with `spitedb_last_error`).
#[no_mangle]
...
///
/// The caller must free the result with `spitedb_free_result`.
#[no_mangle]
pub extern "C" fn spitedb_append_json(
    ...
    if handle.is_null() {
        result.error = make_error_string("handle is null");
        return result;
    }
    ...
    let payload = match unsafe { CStr::from_ptr(json_payload) }.to_str() {
        Ok(s) => s,
        Err(_) => {
            result.error = make_error_string("invalid UTF-8 in payload");
            return result;
        }
    };
    ...
    let req: AppendRequest = match serde_json::from_str(payload) {
        Ok(r) => r,
        Err(e) => {
            result.error = make_error_string(&format!("invalid JSON: {}", e));
            return result;
        }
    };

    if req.events.is_empty() {
        result.error = make_error_string("events array cannot be empty");
        return result;
    }
    ...
    match handle.runtime.block_on(handle.db.append(command)) {
        ...
        Err(e) => {
            result.error = make_error_string(&format!("append failed: {}", e));
        }
    }
...
///
/// Returns a JSON string with results array. Caller must free with `spitedb_free_string`.
#[no_mangle]
...
/// Returns a JSON array of events. Caller must free with `spitedb_free_string`.
#[no_mangle]
    ...
    match handle.runtime.block_on(handle.db.read_stream(
        ...
        Ok(events) => {
            let event_data: Vec<_> = events
                .iter()
                .map(|e| {
                    let data: serde_json::Value = serde_json::from_slice(e.data.as_ref())
...
/// Returns -1 on error.
#[no_mangle]
pub extern "C" fn spitedb_get_stream_revision(
    handle: *mut SpiteDbHandle,
    stream_id: *const c_char,
    tenant: *const c_char,
) -> i64 {
