...
describe("generate", () => {
  it("generates code for a simple aggregate", () => {
    ...
    const validatorsFile = result.files.find((f) => f.path === "validators/counter.ts");
    ...
    const handlersFile = result.files.find((f) => f.path === "handlers/counter.ts");
  ...
  it("generates code for multiple aggregates", () => {
    ...
    expect(result.files.some((f) => f.path === "validators/counter.ts")).toBe(true);
    expect(result.files.some((f) => f.path === "validators/user.ts")).toBe(true);

    // Check handlers for both aggregates
    expect(result.files.some((f) => f.path === "handlers/counter.ts")).toBe(true);
    expect(result.files.some((f) => f.path === "handlers/user.ts")).toBe(true);
  ...
  it("generates proper TypeScript types in validators", () => {
    ...
    const validatorsFile = result.files.find((f) => f.path === "validators/order.ts");
    expect(validatorsFile).toBeDefined();
