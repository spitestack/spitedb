...
async function executeCommand<TInput extends { id: string }>(
  ...
  if (newEvents.length === 0) {
    console.log(`[handler] read=${(tRead-t0).toFixed(2)}ms replay=${(tReplay-tRead).toFixed(2)}ms (no events)`);
    return {
  ...
  console.log(`[handler] read=${(tRead-t0).toFixed(2)}ms replay=${(tReplay-tRead).toFixed(2)}ms serialize=${(tSerialize-tReplay).toFixed(2)}ms persist=${(tPersist-tSerialize).toFixed(2)}ms total=${(tPersist-t0).toFixed(2)}ms`);
